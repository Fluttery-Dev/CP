...
vvi dp;
int helper(int i, int a, int b, vi &arr, vi&brr, vvi& dp){
    if(i == arr.size()) return 0;
    vi x = {a,b};
    if(dp[i][a+b] != -1) return dp[i][a+b];
    if(a+b >= max(arr[i],brr[i])){
        x.push_back(max(arr[i],brr[i]));
        sort(all(x));
        return dp[i][a+b]=1+helper(i+1, x[1], x[2], arr, brr, dp);
    }
    if(a+b >= min(arr[i],brr[i])){
        x.push_back(min(arr[i],brr[i]));
        sort(all(x));
        int mn =  1+helper(i+1, x[1], x[2], arr, brr, dp);

        x = {a,b};
        x.push_back(max(arr[i],brr[i]));
        sort(all(x));
        int mx = helper(i+1, x[1], x[2], arr, brr, dp);
        return dp[i][a+b] = max(mn,mx);
    }
    x.push_back(max(arr[i],brr[i]));
    sort(all(x));
    return dp[i][a+b] = helper(i+1, x[1], x[2], arr, brr, dp);
}
...
void solve(){
    ...
    nbrr;
    dp = vvi(n, vi (4*n+1,-1));
    cout<<helper(2, max(arr[0], brr[0]),max(arr[1], brr[1]), arr, brr, dp)<<endl;
}
