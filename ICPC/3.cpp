...
void solve(){
    int n,k;
    cin>>n>>k;
    vector<pair<int,int>> arr(n+1, {n+1, 0});

    forn(i,k){
        int a,b,m;
        cin>>a>>b>>m;
        arr[m] = {min(arr[m].first, a), max(arr[m].second, b)};
    }
    vi ans(n,1);
    for(int i=2; i<=n; i++){
        arr[i].first = max(arr[i].first, arr[i-1].first);
        arr[i].second = min(arr[i].second, arr[i-1].second);
    }
    forn(i,n){
        auto p = arr[i+1];
        if(p.first == n+1) continue;
