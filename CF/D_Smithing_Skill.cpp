...
void solve(){
    ...
    multiset<int> s;int t;

    forn(i,n) cin>>arr[i].first;
    forn(i,n) cin>>arr[i].second;
    forn(i,m) {
        cin>>t; s.insert(t);
    }

    sort(arr.begin(), arr.end(), [](const pair<int, int>& a, const pair<int, int>& b) {
        return (a.first - a.second) < (b.first - b.second); // Comparator for difference
    });
    ...
    while(i<n){
        ...
        if(x < arr[i].first)i++;
        while(x >= arr[i].first){
            x +=  arr[i].second- arr[i].first ;ans+=2;
        }
        s.erase(it);
        s.insert(x);
    }
