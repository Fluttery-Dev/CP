...
vi arr(1007);
int n;
bool helper(double slope, int p){
    vector<bool> visited(n);
    int t =0;

    forn(i,n){
        
        if((arr[i] - arr[p]) == slope*(i-p)){
            t++;
            visited[i] = true;
        }
    ...
    for(int i= p+1; i<n; i++){
        if(!visited[i])
            if(arr[i]-arr[p] != slope*(i-p)) return false;
    }
    return true;
} 
...
void solve(){
    ...
    possible |= helper( arr[1]-arr[0], 0);
    possible |= helper(arr[2]-arr[1], 1);
    possible |= helper(0.5 *(arr[2]-arr[0]), 0);
