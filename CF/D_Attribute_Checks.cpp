...
vector<vi> dp(5000+5,vi(5000+1,-1));

vi mp;

int helper(int i, int strength, int intelligence, vi &arr, vector<vi> &dp){
    if(i == arr.size()) return 0;
    int j = mp[i];
   if(dp[j][strength] != -1) return dp[j][strength];

   if(arr[i] == 0){
    return dp[j][strength]= max(helper(i+1,strength+1,intelligence,arr, dp),helper(i+1,strength,intelligence+1,arr, dp));
   }
   if(arr[i] < 0){
    return dp[j][strength]= helper(i+1,strength,intelligence,arr, dp) + (strength>=abs(arr[i]));
   }
   return dp[j][strength]= helper(i+1,strength,intelligence,arr, dp) + (intelligence>=arr[i]);
}
