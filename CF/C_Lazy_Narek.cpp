...
int calc(vector<string> &strings, int i, int c, int &n, vvi &dp){
    if(i>=n) return 0;
    ...
    int sc = calc(strings, i+1, c2, n, dp);
    int sc2 = calc(strings, i+1, c, n, dp) + s1-s2;
    return dp[i][c2] = max(sc,sc2);
}
void solve(){
    int n,m;
    cin>>n>>m;
    ...
    cout<<calc(strings, 0, 0, n, dp)<<endl;
