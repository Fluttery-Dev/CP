...
bool check(int mid, unordered_map<int,int> &mp, int k, int &target){
    int total_tasks=0;

    fo(i,1, k+1){
        total_tasks += min(mid, mp[i]+ (mid-mp[i])/2);
        // cout<<min(mid, mp[i]+ (mid-mp[i])/2)<<endl;
    }
    // cout<<mid sp total_tasks sp target<<endl;
    return total_tasks >= target;
}
...
void solve(){
    ...
    while(lo <= hi){
        ...
        if(check(mid, freq, k, total_tasks)){
            ans = mid;
            hi = mid-1;
        }else{
            lo = mid+1;
