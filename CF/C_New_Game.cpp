...
int calc(int i, map<int,int> &mp, int k){
    int ans = 0;
    int sum = 0;
    int t = 0;
...
void solve(){
    ...
    for(auto it = mp.begin(); it!=mp.end(); it++){
        ans = max(ans, calc(it->first, mp, k));
        // cout<<it->first sp calc(it->first, mp, k)<<endl;
        while(it!= mp.end() &&  mp.find(it->first+1) != mp.end()){
            it++;
        }
        // cout<<it->first<<endl;
    }
