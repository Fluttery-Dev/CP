...
void solve(){
    ...
    for(auto& p: mp){
        elementsByCount.push_back(p);
    }

    sort(elementsByCount.begin(), elementsByCount.end(), [](const pair<int, int>& a, const pair<int, int>& b){
        return a.second < b.second; 
    });

    set<int> st;
    for(auto& p: elementsByCount){
        st.insert(p.first); 
    }
