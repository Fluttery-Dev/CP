...
void solve(){
    ...
    sort(all(segments), [](pair<pii,int> &x,  pair<pii,int> &y){
    ...
    forn(i,n){
        auto p = segments[i];
        int pos = s.order_of_key(p.first.second);
        contained[p.second] = (i-pos); 
        s.insert(p.first.second);
    }

    ordered_set_greater st;

    rfo(i,n-1, -1){
        auto p = segments[i];
        int pos = st.order_of_key(p.first.second);
        // cout<<p.first.second sp pos<<endl;
        contains[p.second] = (st.size()-pos); 
        st.insert(p.first.second);
    }
    printv(contains);
    printv(contained);
