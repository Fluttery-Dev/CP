...
int helper(int i, vi &points, vi &b, vi &vis, vector<unordered_map<string, int>> &dp, string path){
    if(i>points.size() || i==0)return 0;
    i--;

    if(dp[i].find(path)!=dp[i].end())return dp[i][path];

    if(vis[i])return helper(i, points, b, vis, dp, path);
    vis[i] = 1;
    path+=to_string(i);
    sort(all(path));
    int score1 = points[i] + helper(i, points, b, vis, dp, path);
    int score2 = helper(b[i], points, b, vis, dp, path);
    vis[i] = 0;
    return dp[i][path] = max(score1, score2);
}
...
void solve(){
    ...
    cout<<helper(1,arr, brr, vis, dp, "")<<endl; 
}
