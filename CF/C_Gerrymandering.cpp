...
int p2(vector<vi> &block){
    int s1 =0, s2 = 0, score = 0;
    s1 += block[0][0] + block[1][0]+ block[0][1];
    s2 += block[0][2] + block[1][2]+ block[1][1];
    if(s1>0) score++;
    if(s2>0) score++;
    return score;
}

int p3(vector<vi> &block){
    int s1 =0, s2 = 0, score = 0;
    s1 += block[0][0] + block[1][0]+ block[1][1];
    s2 += block[0][1] + block[1][2]+ block[0][2];
    if(s1>0) score++;
    if(s2>0) score++;
    return score;
}

int calc(vector<vi> &block){
    
    int ans = 0;
    
    ans = max(ans,p1(block));
    ans = max(ans, p2(block));
    ans = max(ans, p3(block));
    return ans;
}
...
void solve(){
    ...
    cin>>f;
    n = f/3;
    vector<vector<vi>> blocks(n, vector<vi>(2, vi(3,0)));

    forn(j,2)
    forn(i,f){
        char c;
        cin>>c;
        blocks[i/3][j][i%3] = mp[c];
    }
